...
''' 
    ...
    The spx.py module runs the delta energy (dE) experiments, comparing baseline, Multi-Layer Perceptron
    (MLP) Neural Network (NN) performance to Hamiltonian NN (HNN) for the simple pendulum.
    It trains the networks, forecasts orbits via inference mode, calculates 
...
def gen_orbits(spsys, base_model, hnn_model, state0):
    save_state_xyp = np.zeros(3)
    save_state_xyp[0] = np.cos(state0[0])
    save_state_xyp[1] = np.sin(state0[0])
    save_state_xyp[2] = state0[1]
...
if __name__ == "__main__":
  ...
  for power in range(7,16):
    ...
    logmsg("forecast orbits");
    forecast(models, spsys, args, npoints)
  logmsg("done!")
