...
class DynamicalSystem():
    ...
    def __init__(self, sys_hamiltonian, split_ratio=0.3, timesteps=1000,
                 state_symbols=['q', 'p'],
                 no_trajectories=100, tspan=[0, 100], verbose=False, integrator="RK45", symplectic_order=2):
        super(DynamicalSystem, self).__init__()
        self.split = split_ratio
        self.time_points = timesteps
        self.trajectories = no_trajectories
        self.tspan = tspan
        ...
        logmsg(f"DS init.  sys_dim: {self.sys_dim} state sym: {self.state_symbols} sympy sym: {self.sympy_symbols}")
    ...
    def sample_orbits(self):
        ...
        logmsg("generating total points: {:,}".format(N))
        while len(x) < N:
            pct = 100*len(x)/N
            if len(x) % 100 == 0:
                logmsg("% complete: {:.2f}".format(pct))
            state = self.random_config()
    ...
    def make_orbits_dataset(self):
        ...
        for k, v in data.items():
            split_data[k], split_data['test_' + k] = v[split_idx:], v[:split_idx]
        data = split_data
        data['meta'] = orbit_settings
        logmsg(f"after data['coords'].shape: {data['coords'].shape}")
        logmsg(f"after data['test_coords'].shape: {data['test_coords'].shape}")

        return data

    def read_dataset(filename, save_dir):
        '''Reads the input dataset from the given file.'''
        path = '{}/{}'.format(save_dir, filename)
        try:
            data = from_pickle(path)
        except:
            logmsg(f"Cannot load data from {path}")
            exit(-1)
    ...
    def get_dataset(self, filename, save_dir):
        ...
        try:
            ...
            logmsg(f"Cannot load data from {path}")
            logmsg("building dataset")
            data = self.make_orbits_dataset()
            logmsg("dataset build complete")
            to_pickle(data, path)
            xdata = from_pickle(path)
